...
// Get DOM elements
const searchBar = document.getElementById('searchbar'); // Search input element
const mealsDiv = document.getElementById('mealsdiv'); // Container for displaying meals
const searchButton = document.getElementById('searchbtn'); // Button for displaying random image

const myFavoriteMeals = document.getElementById('favoritebar'); // Button for displaying favorite meals
...
function displaySearchRandom() {
    let keyword = searchBar.value.trim(); // Remove leading/trailing whitespace
    if (keyword) {
      URL = `https://www.themealdb.com/api/json/v1/1/search.php?s=${keyword}`;
      createMeals(URL);
    } else {
...
searchButton.addEventListener('click',displaySearchRandom); // search button click event
